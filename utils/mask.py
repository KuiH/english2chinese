...
def tensor_mask(x, valid_len, value=0):
    """
    给x进行mask, 有效长度由valid_len指定。
    x = [[1,2,3],[4,5,6]], valid_len = [1,2] ---> [[1,0,0],[4,5,0]]
    """
    assert x.shape[0] == len(valid_len), "valid_len must have the same length as the first dimension of x"
...
class MaskedCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    def forward(self, pred, label, valid_len):
        ...
        weights = tensor_mask(weights, valid_len)
